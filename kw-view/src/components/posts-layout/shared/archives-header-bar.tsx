...
const ArchivesHeaderBar = ({ currentView, getCurrentView }: PostsHeaderBarProps) => {
  const router = useRouter();
  const id = Number(router.query.id);
  const { data } = useSelector((state: AppState) => state.archives.categories);
    ...
    <div className="post-header-bar">
      {categoryName && (
        <div className="post-header-bar-content">
          <h5>
            Categorías: <span>{categoryName}</span>
          </h5>
        </div>
      )}
...
export default ArchivesHeaderBar;
